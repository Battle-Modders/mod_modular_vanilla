...
Vanilla only sets `corpse.Faction` for a subset of actors i.e. `human`, `skeleton`, `zombie` and the ones dying from `obsidian_dagger.onDamageDealt`. This leads to inconsistencies when mods try to check the faction of a corpse. Modular Vanilla sets the `corpse.Faction` of all corpses generated by actors to the faction of the dying actor.

### `cascade_skill` and `hail_skill` missing null and isAlive checks in `onUse`
Bug report for missing null check: https://steamcommunity.com/app/365360/discussions/1/604158579076817361/
Bug report for missing isAlive check: https://steamcommunity.com/app/365360/discussions/1/684112727828878904/
